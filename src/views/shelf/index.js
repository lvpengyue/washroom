...
export default {
  ...
  data() {
    // 定义所有按钮的名称，与权限挂钩
    const showEdit = Util.showThisRoute(JSON.parse(Cookies.get('permission')), 'shelf_index_edit') ? 'inline-block' : 'none';
    const showDel = Util.showThisRoute(JSON.parse(Cookies.get('permission')), 'shelf_index_del') ? 'inline-block' : 'none';
