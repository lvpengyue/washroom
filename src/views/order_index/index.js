...
export default {
  ...
  data() {
    // 定义所有按钮的名称，与权限挂钩
    // 查看衣物列表
    const showClothes = Util.showThisRoute(JSON.parse(Cookies.get('permission')), 'order_index_clothes') ? 'inline-block' : 'none';
    // 展示返洗列表
    const showReturn= Util.showThisRoute(JSON.parse(Cookies.get('permission')), 'order_index_return') ? 'inline-block' : 'none';
