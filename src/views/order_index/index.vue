...
<template>
    <div class="orderIndex-list" v-if="orderIndexData && orderIndexData.data">
        <Row>
            <Col span="24">
            <Card>
                <div slot="title">
                    <Form inline ref="search" :model="search">
                        ...
                        <FormItem>
                            <Select v-model="search.washType" style="width:200px" placeholder="请选择订单类型">
                                <Option :value="index" :key="index" v-for="(item, index) in typeArr">{{item}}</Option>
                            </Select>
                        </FormItem>
                        <FormItem>
                            <Button type="primary"
